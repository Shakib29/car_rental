...
const MumbaiLocalBooking: React.FC = () => {
  ...
  useEffect(() => {
    if (booking.pickup && booking.dropoff && booking.pickup !== booking.dropoff) {
      // Simulate distance calculation with random values for demo
      const simulatedDistance = Math.floor(Math.random() * 50) + 5;
      setDistance(simulatedDistance);
      
      // Calculate price based on distance and service type
      let baseRate = 15; // per km
      if (booking.serviceType === 'airport-pickup' || booking.serviceType === 'airport-drop') {
        baseRate = 18; // Airport transfers cost more
      }
      
      const calculatedPrice = Math.round(simulatedDistance * baseRate);
      setPrice(calculatedPrice);
    } else {
      setDistance(0);
      setPrice(0);
    }
  }, [booking.pickup, booking.dropoff, booking.serviceType]);
            ...
            <div className="grid md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                >
                  ...
                  {popularLocations.filter(loc => loc !== booking.pickup).map(location => (
                    <option key={location} value={location}>{location}</option>
                  ))}
                </select>
              </div>
